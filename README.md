...
## Button/LCD desk interface

Arduino UNO R4 WiFi button/LCD board runs `arduino_codes/button_lcd/button_lcd.ino`.
It keeps the fixed Mobius topology and uses:
...
- line 1: `SYSTEM READY`
- line 2: `PRESS BUTTON A`

### Data flow

1. Start FastAPI with `uvicorn app.main:app --reload`.
2. Start the Raspberry Pi watcher with `python3 raspberry_pi_codes/main.py`. It waits for a new `postureCamera/command` with `action=start`.
3. Power the Arduino button/LCD board.
4. Button A posts a START event to `deskInterface/buttonEvents`.
5. FastAPI receives the Mobius notification and creates a `postureCamera/command` START command.
6. Raspberry Pi receives START, opens the camera, analyzes posture, and posts samples/events to `postureCamera/postureSamples` and `postureCamera/postureEvents`.
7. FastAPI maps posture analysis to `deskInterface/lcdCommand`.
8. Arduino polls the latest LCD command in about 1 second and updates the 16x2 LCD.
9. If the LCD command enables acceptance, button B posts an ACCEPT event.
10. FastAPI creates `deskMotor/command` only after ACCEPT. The motor Arduino polls `GET /deskMotor/command/latest` and moves to STAND for `target_height_cm >= 100`, otherwise SIT.

### Payloads

Button A START payload:
...
Posture LCD command payload:

```json
{
  "command_id": "lcd-command-id",
  "session_id": "active-session-id",
  "type": "posture_result",
  "posture_state": "TURTLE_NECK",
  "line1": "STANDING MODE",
  "line2": "PRESS B TO OK",
  "accept_enabled": true,
  "target_height_cm": 125
}
```

Button B ACCEPT payload:
...
Motor command created by the backend after ACCEPT:

```json
{
  "command_id": "motor-command-id",
  "session_id": "active-session-id",
  "action": "set_height",
  "target_height_cm": 125
}
...
- `NORMAL`: no desk movement request, button B disabled.
- `DESK_HEIGHT_CHANGE` maps to `TURTLE_NECK`: LCD asks for standing mode, button B enabled.
- `POSTURE_CORRECTION` maps to `STRETCH_WARNING`: LCD shows stretch/posture warning, button B disabled.

### Button/LCD wiring

| Part | Arduino UNO R4 WiFi pin |
| --- | --- |
| Button A one side | D2 |
| Button B one side | D3 |
| Button A/B other side | GND |
| LCD VCC | 5V |
| LCD GND | GND |
| LCD SDA | SDA |
| LCD SCL | SCL |

Buttons use `INPUT_PULLUP`, so pressed state is LOW. LCD default I2C address is `0x27`.
